...
workqueue_mon_test: $(CCTOOLS_INSTALL)  ## run all tests with workqueue_ex config
	...
	date
	ls -l runinfo/ || true
	PATH=.:$(PATH) PYTHONPATH=.:/tmp/cctools/lib/python3.8/site-packages  pytest parsl/tests/test_python_apps/ -k "not cleannet and not issue363" --config parsl/tests/configs/workqueue_monitoring_config.py --cov=parsl --cov-append --cov-report= 
