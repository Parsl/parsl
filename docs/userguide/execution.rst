...
1. ``tasks_per_node``: Number of tasks that can execute per node (which corresponds to the number of workers started per node).
...
is expressed as the ratio of resource capacity and active tasks.
