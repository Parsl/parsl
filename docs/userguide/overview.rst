...
in the list `mapped_results`; this list is provided as ainput to the `app_sum` app, which therefore cannot execute until all four values are available; and finally, the `app_sum` app computes the sum of the doubled values.
