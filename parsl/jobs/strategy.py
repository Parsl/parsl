...
# TODO: how does this get its DFK scope? pass it in at init, and avoid using a
# module level logger entirely?
...
class Strategy:
    ...
    def _general_strategy(self, executors: List[BlockProviderExecutor], *, strategy_type: str) -> None:
        ...
        for executor in executors:
            self._general_strategy_for_executor(executor, strategy_type)

    @wrap_with_logs
    def _general_strategy_for_executor(self, executor, strategy_type):
        label = executor.label
        with LexicalSpan(logger, "strategizing for executor {label}"):
            # TODO ability to add extra attributes here, usin with span as new_logger. and then logging
            # to new_logger, a suitable adapter

            if executor.bad_state_is_set:
                logger.info(f"Not strategizing for executor {label} because bad state is set", extra={"parsl_executor": label})
                return

            logger.debug(f"Strategizing for executor {label}", extra={"parsl_executor": label})

            if self.executors[label]['first']:
                logger.debug(f"Scaling out {executor.provider.init_blocks} initial blocks for {label}", extra={"parsl_executor": label})
                executor.scale_out_facade(executor.provider.init_blocks)
            ...
            logger.debug(f"Slot ratio calculation: active_slots = {active_slots}, active_tasks = {active_tasks}", extra={"parsl_executor": label, "active_slots": active_slots, "active_tasks": active_tasks, "metric": "slot_ratio"})

            logger.debug('Executor {} has {} active tasks and {}/{} running/pending blocks'.format(
                label, active_tasks, running, pending), extra={"parsl_executor": label, "active_tasks": active_tasks, "blocks_running": running, "blocks_pending": pending})
            ...
            if active_tasks == 0:
                # Case 1a
                logger.debug("Strategy case 1: Executor has no active tasks", extra={"parsl_executor": label})

                # Fewer blocks that min_blocks
                if active_blocks <= min_blocks:
                    logger.debug("Strategy case 1a: Executor has no active tasks and minimum blocks. Taking no action.", extra={"parsl_executor": label})
                # Case 1b
                # More blocks than min_blocks. Scale in
                    ...
                    logger.debug(f"Strategy case 1b: Executor has no active tasks, and more ({active_blocks})"
                                 f" than minimum blocks ({min_blocks})", extra={"parsl_executor": label, "blocks_active": active_blocks, "blocks_min": min_blocks})

                    if not self.executors[executor.label]['idle_since']:
                        logger.debug(f"Starting idle timer for executor. If idle time exceeds {self.max_idletime}s, blocks will be scaled in", extra={"parsl_executor": label, "max_idletime": self.max_idletime})
                    ...
                    if idle_duration > self.max_idletime:
                        ...
                        logger.debug(f"Idle time has reached {self.max_idletime}s for executor {label}; scaling in", extra={"parsl_executor": label, "max_idletime": self.max_idletime})
                        ...
                        logger.debug(
                                ...
                                f" for executor {label}; not scaling in", extra={"parsl_executor": label, "max_idletime": self.max_idletime, "idle_duration": idle_duration})

            # Case 2
            # More tasks than the available slots.
            elif (float(active_slots) / active_tasks) < parallelism:
                logger.debug("Strategy case 2: slots are overloaded - (slot_ratio = active_slots/active_tasks) < parallelism", extra={"parsl_executor": label})

                # Case 2a
                # We have the max blocks possible
                if active_blocks >= max_blocks:
                    # Ignore since we already have the max nodes
                    logger.debug(f"Strategy case 2a: active_blocks {active_blocks} >= max_blocks {max_blocks} so not scaling out", extra={"parsl_executor": label, "blocks_active": active_blocks, "blocks_max": max_blocks})
                # Case 2b
                else:
                    logger.debug(f"Strategy case 2b: active_blocks {active_blocks} < max_blocks {max_blocks} so scaling out", extra={"parsl_executor": label, "blocks_active": active_blocks, "blocks_max": max_blocks})
                    ...
                    logger.debug(f"Requesting {excess_blocks} more blocks", extra={"parsl_executor": label, "excess_blocks": excess_blocks})
                    executor.scale_out_facade(excess_blocks)

            elif active_slots == 0 and active_tasks > 0:
                logger.debug("Strategy case 4a: No active slots but some active tasks - could scale out by a single block", extra={"parsl_executor": label})

                # Case 4a
                if active_blocks < max_blocks:
                    logger.debug("Requesting single block", extra={"parsl_executor": label})

                    executor.scale_out_facade(1)
                else:
                    logger.debug("Not requesting single block, because at maxblocks already", extra={"parsl_executor": label})

            # Case 4b
            # More slots than tasks
            elif active_slots > 0 and active_slots > active_tasks:
                logger.debug("Strategy case 4b: more slots than tasks", extra={"parsl_executor": label})
                if strategy_type == 'htex':
                    ...
                    if isinstance(executor, HighThroughputExecutor):
                        if active_blocks > min_blocks:
                            ...
                            logger.debug(f"Requesting scaling in by {excess_blocks} blocks with idle time {self.max_idletime}s", extra={"parsl_executor": label, "excess_blocks": excess_blocks})
                            executor.scale_in_facade(excess_blocks, max_idletime=self.max_idletime)
                    else:
                        logger.error("This strategy does not support scaling in except for HighThroughputExecutor - taking no action", extra={"parsl_executor": label})
                else:
                    logger.debug("This strategy does not support scaling in", extra={"parsl_executor": label})

            # Case 3
            # tasks ~ slots
            else:
                logger.debug("Strategy case 3: no changes necessary to current block load", extra={"parsl_executor": label, "strategy_case": "3"})  # TODO: label up other strategy cases
