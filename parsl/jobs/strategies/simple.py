...
class SimpleStrategy(InitOnlyStrategy):
    ...
    def __init__(self, executor: "BlockProviderExecutor", max_idletime: float) -> None:
    ...
    def no_tasks(self, state: StrategyState) -> bool:
        ...
        if self._executor_idle_since is None:
            logger.debug(
                ...
                self.build_prefix(), self.max_idletime
            )
        ...
        if idle_duration >= self.max_idletime:
            ...
            logger.debug(
                ...
                self.build_prefix(), idle_duration, self.max_idletime
            )

        return True

    def needs_more_slots(self, state: StrategyState) -> bool:
        if state.active_tasks <= 0:
            return False

        # Case 3
        # Degenerate case: tasks exist, but the configured parallelism leads to
        # zero desired slots. In this situation, request a single block to allow
        # progress/bootstrap.
        if state.parallelism == 0:
            ...
            logger.debug(
                "%s Strategy case 3: no active slots and parallelism=0; "
            ...
            if state.active_blocks < state.max_blocks:
                ...
                logger.debug(
                    "%s Not requesting any blocks, because at maxblocks already",
                    self.build_prefix(),
                )
        ...
        logger.debug(
            "%s Strategy case 2: slots are overloaded - "
            "(slot_ratio = active_slots/active_tasks) < parallelism",
            self.build_prefix(),
        )
