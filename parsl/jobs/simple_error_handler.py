import parsl.executors.base as executors_base
...
def simple_error_handler(executor: executors_base.ParslExecutor, status: Dict[str, JobStatus], threshold: int):
...
# only used by simple_error_handler -  so should go wherever that goes
...
# BENC: only used by simple_error_handler - so should go wherever that goes
