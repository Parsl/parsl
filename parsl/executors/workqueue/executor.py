    ...
    while(continue_running):
        ...
        while task_queue.qsize() > 0:
            ...
            if function_name not in environment_table.keys():
                ...
                output_json_file = "task_" + function_name + "_analysis.json"
                environment_name = function_name + "_venv"
                environment_path = os.path.abspath(environment_name + ".tar.gz")
                analysis_exit = subprocess.call("{} {} {}".format(python_package_analyze, function_source_loc, output_json_file), shell=True)
                creation_exit = subprocess.call("{} {} {}".format(python_package_create, output_json_file, environment_name), shell=True)
                logger.debug("Python package analysis exit code: {}\nPython package creation exit code: {}".format(analysis_exit, creation_exit))
