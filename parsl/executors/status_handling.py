...
class BlockProviderExecutor(ParslExecutor):
    ...
    def scale_out_facade(self, n: int) -> List[str]:
        ...
        logger.info(f"%s Scaling out by {n} blocks", self._scale_prefix, extra={"parsl_executor": self.label, "count": n})
        for _ in range(n):
            block_id = str(self._block_id_counter.get_id())
            logger.info(f"%s Allocated block ID {block_id}", self._scale_prefix, extra={"parsl_executor": self.label, "block_id": block_id})
            try:
