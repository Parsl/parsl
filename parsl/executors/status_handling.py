...
class BlockProviderExecutor(ParslExecutor):
    ...
    def send_monitoring_info(self, status: Dict) -> None:
        ...
        for bid, s in status.items():
            logger.debug(f"block ID {bid}, new status {s}", extra={"parsl_dfk": self.run_id, "parsl_executor": self.label, "block_id": bid, "job_id": self.blocks_to_job_id.get(bid, None), "status": s.status_name})
