...
class TorqueProvider(ClusterProvider, RepresentationMixin):
    ...
    def _status(self):
        ...
        retcode, stdout, stderr = self.channel.execute_wait("qstat {0}".format(job_id_list))
    ...
    def submit(self, command, blocksize, tasks_per_node, job_name="parsl.auto"):
        ...
        retcode, stdout, stderr = self.channel.execute_wait(launch_cmd)
    ...
    def cancel(self, job_ids):
        ...
        retcode, stdout, stderr = self.channel.execute_wait("qdel {0}".format(job_id_list))
        rets = None
        if retcode == 0:
            for jid in job_ids:
