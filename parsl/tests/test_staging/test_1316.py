...
class SP1(Staging):
    def can_stage_in(self, file):
        return file.scheme == 'sp1'

    def stage_in(self, dm, executor, file, parent_fut):
        file.local_path = "./test1.tmp"
        return file  # specify an updated File object to be used on the app side
...
@pytest.mark.local
def test_1316_local_path_on_execution_side():
    ...
    config = Config(executors=[ThreadPoolExecutor(storage_access=[SP1()])])
...
@pytest.mark.local
def test_1316_local_path_setting_preserves_dependency():
    config = Config(executors=[ThreadPoolExecutor(storage_access=[SP1()])])
