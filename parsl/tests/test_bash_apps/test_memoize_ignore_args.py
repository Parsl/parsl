...
def test_memo_same_at_definition():
    x = test_app(x=0).result()  # this should be executed
    y = test_app(x=1).result()  # this should be memoized, and will raise a RuntimeError if actually executed
...
def test_memo_stdout():
    ...
    x = test_no_checkpoint_stdout(stdout=path_x).result()
    ...
    y = test_no_checkpoint_stdout(stdout=path_y).result()
    assert not os.path.exists(path_y)
