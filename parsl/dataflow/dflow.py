...
class DataFlowKernel:
    ...
    def launch_task(self, task_record: TaskRecord) -> Future:
        ...
        if hasattr(exec_fu, "parsl_executor_task_id"):
            tl.info(
                f"try %s launched on executor %s "
                f"with executor id %s", try_id, executor.label, exec_fu.parsl_executor_task_id, extra={"parsl_try_id": try_id, "parsl_executor": executor.label, "parsl_executor_id": exec_fu.parsl_executor_task_id})
            # TODO: extra info should include try id here. or it should be
            # in every log as part of the task logger?
